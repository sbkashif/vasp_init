#!/usr/bin/env python3
"""CLI tool: add an H2 molecule to a POSCAR using a TraPPE .def file.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Add H2 between two Cartesian points in a POSCAR")
    parser.add_argument("--poscar", required=True, help="Path to input POSCAR/CONTCAR")
    parser.add_argument("--def", dest="def_path", required=True, help="Path to TraPPE H2 .def file")
    ...
    parser.add_argument("--flags", type=_parse_tff, default=None, help="Selective dynamics flags for added H2 atoms, e.g., TTT")
